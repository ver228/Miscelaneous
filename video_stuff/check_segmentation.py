...
class ReadVideoffmpeg:
    def __init__(self, fileName, width = -1, height = -1):
        ...
        command = [ffmpeg_cmd, 
           ...
           '-f', 'image2pipe',
           '-vcodec', 'rawvideo', '-']
        self.pipe = sp.Popen(command, stdout = sp.PIPE, stderr=sp.PIPE, bufsize = self.tot_pix) #use a buffer size as small as possible, makes things faster
...
def getImageROI(image):
    ...
    #mask = cv2.erosion(mask, np.ones((5,5),np.uint8), iterations = 3)
...
if __name__ == '__main__':
    MAX_N_PROCESSES = 4 + mp.cpu_count()
